...
export function AIExportPage({ transactions }: AIExportPageProps) {
    ...
    const instructionText = `Роль: Действуй как топовый Wealth Manager, финансовый советник и Data-аналитик. Твой подход — откровенный, профессиональный и опирающийся исключительно на цифры. Никакой «воды» и банальных советов. 

Контекст:
В прикрепленном JSON-файле находится полная история моих финансовых транзакций. 
Моя базовая валюта дохода — рубли (RUB), но я живу в Таиланде (Паттайя) по DTV визе, активно путешествую по Азии (Филиппины, Индонезия, Китай) и совершаю траты в THB, USDT и других локальных валютах. Я работаю на удаленке (сочетая стандартный график со сменным 2/2), активно использую криптовалюту, P2P-обмен (Bybit) и экосистему Apple. В жизни и путешествиях я ценю жесткое планирование, порядок и подход «больше за меньше».

Задача:
Проанализируй данные и предоставь максимально глубокую, реальную картину моей финансовой жизни. Я хочу знать, где мои проблемы, где я теряю деньги и как мне кратно вырасти.

Структура анализа:
1. Аудит расходов и «Финансовые дыры»: Детально разбери мои траты. В каких категориях я переплачиваю? Есть ли у меня скрытые утечки бюджета или импульсивные траты, которые маскируются под «базовые потребности»?
2. Оценка дельты и уязвимостей: Проанализируй мою норму сбережений (Savings Rate). Насколько мой бюджет уязвим к колебаниям курса (RUB/USDT/THB) и кросс-курсовым потерям при конвертации?
3. Прогноз на 3 месяца (Predictive Analysis): Опираясь на исторические паттерны из файла, построй реалистичный прогноз моего баланса и расходов на следующий квартал. Что будет, если я ничего не изменю в своих привычках?
4. Зоны роста и стратегия капитала: Учитывая мой интерес к крипте, защите от инфляции и автоматизации, какие конкретные шаги мне нужно предпринять для оптимизации капитала? Как эффективнее распределять свободные средства?
5. Оценка качества данных: Оцени структуру самого JSON-файла. Насколько чисто и грамотно я веду учет? Чего не хватает в моих логах для идеальной финансовой аналитики?

Формат вывода:
Используй четкую иерархию, маркированные списки и таблицы для сравнения категорий. Выделяй ключевые инсайты и цифры жирным шрифтом. Я жду суровую правду и конкретные, выполнимые рекомендации.`;
